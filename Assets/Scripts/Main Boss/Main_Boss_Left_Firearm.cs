...
public class Main_Boss_Left_Firearm : MonoBehaviour
{
    [SerializeField]
    ...
    void Update()
    {
        if(player._isPlayerAlive && _player.transform.position.x<0)
        {
            RotateTowardsPlayer();
        }
        else
    ...
    IEnumerator FireBulletRoutine()
    {
        ...
        {
            Instantiate(_bulletPrefab,intPos.transform.position, transform.rotation);
